...
export const PROVIDER_CONFIGS: Record<
  string,
  Record<string, ProviderRateLimitConfig>
> = {
  openai: {
...
export class RateLimiter {
  ...
  private calculateExponentialBackoff(
    ...
    if (tracker.recentDelays.length > 0) {
      const avgRecentDelay =
        tracker.recentDelays.reduce((sum, d) => sum + d, 0) /
  ...
  public shouldThrottle(
    ...
    if (
      providerInfo.concurrencyLimit &&
      tracker.activeRequests >= providerInfo.concurrencyLimit
    ) {
      return {
        throttle: true,
        recommendedDelay: this.calculateThrottleDelay(
          0.9,
          tracker,
          TimeWindow.MINUTE,
        ),
  ...
  private getProviderConfig(provider: string): ProviderRateLimitConfig | null {
    ...
    return (
      providerConfigs["default"] || Object.values(providerConfigs)[0] || null
    );
  }
...
export async function executeWithRateLimiting<T>(
  ...
  const throttleCheck = limiter.shouldThrottle(
    provider,
  ...
  if (throttleCheck.throttle) {
    ...
    await Promise.resolve(
      new Promise((resolve) =>
        setTimeout(resolve, throttleCheck.recommendedDelay),
      ),
    );
  }
